...
impl application_framework::Application for Application {
    fn new(device: &wgpu::Device, _queue: &mut wgpu::Queue, swap_chain_descriptor: &wgpu::SwapChainDescriptor) -> Self {
        ...
        paths[0].stroke_options = Some(contrast_renderer::path::StrokeOptions {
            width: 0.1,
            offset: 0.0,
            join: contrast_renderer::path::Join::Bevel,
            cap: contrast_renderer::path::Cap::Closed,
            curve_approximation: contrast_renderer::path::CurveApproximation::UniformTangentAngle(0.2),
        });
    ...
    fn render(&mut self, device: &wgpu::Device, queue: &mut wgpu::Queue, frame: &wgpu::SwapChainTexture) {
        let transform_uniform_data = glam::Mat4::perspective_lh(
            ...
            1.0,
            1000.0,
        ) * glam::Mat4::from_rotation_translation(self.view_rotation, glam::vec3(0.0, 0.0, 5.0));
        self.renderer.set_transform(&queue, &transform_uniform_data);
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
        let clip_stack = contrast_renderer::renderer::ClipStack::default();
        {
        ...
        }
        {
            ...
            self.fill_shape.render_color_solid(&self.renderer, &mut render_pass, clip_stack.height());
        }
